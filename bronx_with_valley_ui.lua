...
-- ===== Appended Valley UI Module =====

-- ==== VALLEY.GG STYLE CYBER UI MODULE ====
-- This module REPLACES the existing getgenv().library with a Valley-style UI.
-- It creates a dark centered window with:
--  - left sidebar with tab buttons
--  - top search bar
--  - two main content columns
-- The module exposes the compatibility API used by Bronx: library:window(...)->window:tab(...)->tab:column()->section controls.

do
    local RunService = game:GetService("RunService")
    local UIS = game:GetService("UserInputService")

    local Fonts = (getgenv().library and getgenv().library.Fonts) or {}
    local GetImage = getgenv().GetImage -- use existing GetImage if present

    local newlib = {}
    newlib.directory = (getgenv().library and getgenv().library.directory) or "bronx.lol_remastered"
    newlib.themes = {
        bg = Color3.fromRGB(18,18,20),
        panel = Color3.fromRGB(28,28,30),
        accent = Color3.fromRGB(40,200,255),
    ...
    local function parentGui(gui)
        local ok, container = pcall(function() return gethui() end)
    ...
    function newlib:window(opts)
        opts = opts or {}
        local title = opts.name or "Valley.gg"
        local size = opts.size or {0, 820, 0, 560}

        local screen = new("ScreenGui")
        screen.Name = "valley_ui"
        screen.ResetOnSpawn = false
        parentGui(screen)

        local main = new("Frame")
        main.Name = "Main"
        main.Size = UDim2.new(0, size[2], 0, size[4])
        main.Position = UDim2.new(0.5, -size[2]/2, 0.5, -size[4]/2)
        main.AnchorPoint = Vector2.new(0.5,0.5)
        main.BackgroundColor3 = newlib.themes.bg
        main.BorderSizePixel = 0
        main.Parent = screen

        local stroke = new("UIStroke", main)
        stroke.Thickness = 2
        stroke.Color = newlib.themes.accent
        ...
        local title_lbl = new("TextLabel", header)
        title_lbl.Size = UDim2.new(0.5, -12, 1, 0)
        title_lbl.Position = UDim2.new(0, 12, 0, 0)
        title_lbl.BackgroundTransparency = 1
        title_lbl.Text = title
        title_lbl.Font = Enum.Font.SourceSansBold
        title_lbl.TextSize = 18
        title_lbl.TextColor3 = newlib.themes.text
        title_lbl.TextXAlignment = Enum.TextXAlignment.Left

        local search_box = new("TextBox", header)
        search_box.Size = UDim2.new(0.4, -16, 0, 28)
        search_box.Position = UDim2.new(0.5, 0, 0, 10)
        search_box.BackgroundColor3 = Color3.fromRGB(22,22,25)
        search_box.PlaceholderText = "Search"
        search_box.Text = ""
        search_box.ClearTextOnFocus = false
        search_box.TextColor3 = newlib.themes.muted
        search_box.BorderSizePixel = 0

        -- body split: left sidebar (tabs) and right content area
        ...
        local side_list = new("UIListLayout", sidebar)
        side_list.Padding = UDim.new(0, 8)
        side_list.HorizontalAlignment = Enum.HorizontalAlignment.Left
        side_list.SortOrder = Enum.SortOrder.LayoutOrder
        side_list.Padding = UDim.new(0, 12)

        -- tabs container on the right
        local content = new("Frame", body)
        content.Size = UDim2.new(1, -196, 1, 0)
        content.Position = UDim2.new(0, 196, 0, 0)
        ...
        footer.BackgroundColor3 = Color3.fromRGB(12,12,14)
        ...
        -- createTab function
        function window_obj:create_tab(tabname)
            local index = #window_obj.tabs + 1
            -- sidebar button
            local btn = new("TextButton", sidebar)
            btn.Name = "TabBtn_"..tabname
            btn.Size = UDim2.new(1, -20, 0, 42)
            btn.Position = UDim2.new(0, 10, 0, (index-1)*46 + 12)
            btn.BackgroundTransparency = 1
            btn.Text = "   "..tostring(tabname)
            btn.TextSize = 14
            btn.Font = Enum.Font.SourceSans
            btn.TextColor3 = newlib.themes.text
            btn.TextXAlignment = Enum.TextXAlignment.Left
            btn.BorderSizePixel = 0

            local accent = new("Frame", btn)
            accent.Size = UDim2.new(0, 6, 1, 0)
            accent.Position = UDim2.new(0, 0, 0, 0)
            accent.BackgroundColor3 = Color3.fromRGB(24,24,28)
            accent.BorderSizePixel = 0

            -- content panel
            local panel = new("Frame", content)
            panel.Size = UDim2.new(1, 0, 1, 0)
            panel.Position = UDim2.new(0,0,0,0)
            panel.BackgroundTransparency = 1
            panel.Visible = false

            local col_left = new("Frame", panel)
            col_left.Size = UDim2.new(0.5, -8, 1, -8)
            col_left.Position = UDim2.new(0, 8, 0, 4)
            col_left.BackgroundTransparency = 1

            local col_right = new("Frame", panel)
            col_right.Size = UDim2.new(0.5, -8, 1, -8)
            col_right.Position = UDim2.new(0.5, 8, 0, 4)
            col_right.BackgroundTransparency = 1
            ...
            -- section factory
            function tab:section(side, section_name)
                side = side or "left"
                local container = (side=="left") and tab.left or tab.right
                local frame = new("Frame", container)
                frame.Size = UDim2.new(1, 0, 0, 150)
                frame.BackgroundColor3 = Color3.fromRGB(20,20,24)
                frame.BorderSizePixel = 0
                frame.AnchorPoint = Vector2.new(0,0)
                frame.Position = UDim2.new(0, 0, (#tab.sections)*0.28, 8)

                local title = new("TextLabel", frame)
                title.Size = UDim2.new(1, -12, 0, 22)
                title.Position = UDim2.new(0, 8, 0, 6)
                title.BackgroundTransparency = 1
                title.Text = tostring(section_name or "Section")
                title.TextSize = 14
                title.Font = Enum.Font.SourceSansBold
                title.TextColor3 = newlib.themes.text
                title.TextXAlignment = Enum.TextXAlignment.Left

                local content_area = new("Frame", frame)
                content_area.Size = UDim2.new(1, -16, 1, -40)
                content_area.Position = UDim2.new(0, 8, 0, 32)
                content_area.BackgroundTransparency = 1

                local section = {frame = frame, area = content_area}

                -- controls

                function section:label(txt)
                    local lbl = new("TextLabel", content_area)
                    lbl.Size = UDim2.new(1, 0, 0, 18)
                    lbl.BackgroundTransparency = 1
                    lbl.Text = tostring(txt or "")
                    lbl.TextSize = 13
                    lbl.Font = Enum.Font.SourceSans
                    lbl.TextColor3 = newlib.themes.muted
                    lbl.TextXAlignment = Enum.TextXAlignment.Left
                    return lbl
                end

                function section:button(cfg)
                    local btn = new("TextButton", content_area)
                    btn.Size = UDim2.new(1, 0, 0, 28)
                    btn.BackgroundColor3 = Color3.fromRGB(32,32,36)
                    btn.BorderSizePixel = 0
                    btn.Text = tostring(cfg.name or "Button")
                    btn.Font = Enum.Font.SourceSans
                    btn.TextSize = 14
                    btn.TextColor3 = newlib.themes.text
                    btn.MouseButton1Click:Connect(function()
                        pcall(function() if cfg and cfg.callback then cfg.callback() end end)
                    end)
                    return btn
                end

                function section:toggle(cfg)
                    local holder = new("Frame", content_area)
                    ...
                    label.Text = tostring(cfg.name or "Toggle")
                    label.TextColor3 = newlib.themes.text
                    label.Font = Enum.Font.SourceSans
                    label.TextSize = 14
                    label.TextXAlignment = Enum.TextXAlignment.Left

                    local sw = new("TextButton", holder)
                    sw.Size = UDim2.new(0.22, -6, 0.8, 0)
                    sw.Position = UDim2.new(0.78, 0, 0.1, 0)
                    sw.BackgroundColor3 = Color3.fromRGB(22,22,28)
                    sw.BorderSizePixel = 0
                    sw.Text = cfg.default and "ON" or "OFF"
                    sw.Font = Enum.Font.SourceSans
                    sw.TextSize = 12
                    sw.TextColor3 = cfg.default and newlib.themes.accent or newlib.themes.muted
                    local state = cfg.default or false
                    sw.MouseButton1Click:Connect(function()
                        state = not state
                        sw.Text = state and "ON" or "OFF"
                        sw.TextColor3 = state and newlib.themes.accent or newlib.themes.muted
                        pcall(function() if cfg.callback then cfg.callback(state) end end)
                    end)
                    return {set = function(v) state = v; sw.Text = v and "ON" or "OFF"; sw.TextColor3 = v and newlib.themes.accent or newlib.themes.muted end}
                end
                ...
                function section:slider(cfg)
                    ...
                    local lbl = new("TextLabel", holder)
                    lbl.Size = UDim2.new(0.6, 0, 0, 18)
                    lbl.Position = UDim2.new(0,0,0,0)
                    lbl.BackgroundTransparency = 1
                    lbl.Text = tostring(cfg.name or "Slider")
                    lbl.TextColor3 = newlib.themes.text
                    lbl.Font = Enum.Font.SourceSans
                    lbl.TextSize = 13
                    lbl.TextXAlignment = Enum.TextXAlignment.Left

                    local value_lbl = new("TextLabel", holder)
                    value_lbl.Size = UDim2.new(0.4, -6, 0, 18)
                    value_lbl.Position = UDim2.new(0.6, 6, 0, 0)
                    value_lbl.BackgroundTransparency = 1
                    value_lbl.Text = tostring(default)
                    value_lbl.TextColor3 = newlib.themes.muted
                    value_lbl.Font = Enum.Font.SourceSans
                    value_lbl.TextSize = 13
                    ...
                    fill.BackgroundColor3 = newlib.themes.accent
                    ...
                    RunService.RenderStepped:Connect(function()
                        if dragging then
                            ...
                            value_lbl.Text = tostring(val)
                            pcall(function() if cfg.callback then cfg.callback(val) end end)
                        end
                    end)
                    return {set = function(v) local pct = (v-min)/(max-min); fill.Size = UDim2.new(pct,0,1,0); value_lbl.Text = tostring(v) end}
                end
                ...
                function section:dropdown(cfg)
                    ...
                    lbl.BackgroundTransparency = 1
                    lbl.Text = tostring(cfg.name or "Dropdown")
                    lbl.Font = Enum.Font.SourceSans
                    lbl.TextColor3 = newlib.themes.text

                    local cur = new("TextButton", holder)
                    cur.Size = UDim2.new(0.4, -6, 1, 0)
                    cur.Position = UDim2.new(0.6, 6, 0, 0)
                    cur.Text = tostring(cfg.default or (items[1] or ""))
                    cur.Font = Enum.Font.SourceSans
                    cur.TextColor3 = newlib.themes.muted
                    cur.MouseButton1Click:Connect(function()
                        -- cycle
                        local idx = 1
                        for i,v in ipairs(items) do if tostring(v)==tostring(cur.Text) then idx = i; break end end
                        idx = idx + 1
                        if idx > #items then idx = 1 end
                        cur.Text = tostring(items[idx])
                        pcall(function() if cfg.callback then cfg.callback(cur.Text) end end)
                    end)
                    return {set = function(newitems) items = newitems end}
                end
            ...
            btn.MouseButton1Click:Connect(function()
                for _,t in ipairs(window_obj.tabs) do
                    t.panel.Visible = false
                end
                panel.Visible = true
                window_obj.current = tab
            end)

            table.insert(window_obj.tabs, tab)
            return tab
        end

        -- expose api similar to original library
        getgenv().library = newlib

        -- compatibility object returned by :window in many scripts
        local compat = {}
        compat.window = screen
        compat.create = function() end
        function compat:tab(cfg)
            -- accept cfg.name or cfg.tabs as list
            if type(cfg) == "table" and cfg.tabs then
                local tabs = {}
                for i,name in ipairs(cfg.tabs) do
                    tabs[i] = window_obj:create_tab(name)
                end
                return unpack(tabs)
            else
                local name = (type(cfg)=="string" and cfg) or (cfg.name or "Main")
                return window_obj:create_tab(name)
            end
        end

        function compat:notification(t)
            newlib.notifications = newlib.notifications or {}
            newlib.notifications.create = function(cfg)
                cfg = cfg or {}
                local s = new("ScreenGui"); parentGui(s)
                local frame = new("Frame", s)
                frame.Size = UDim2.new(0,320,0,64)
                frame.Position = UDim2.new(1, -340, 0, 12)
                frame.BackgroundColor3 = Color3.fromRGB(10,10,12)
                local txt = new("TextLabel", frame)
                txt.Size = UDim2.new(1, -24, 1, -12)
                txt.Position = UDim2.new(0, 12, 0, 6)
                txt.BackgroundTransparency = 1
                txt.Text = (cfg.title or "Notice").."\n"..(cfg.text or "")
                txt.TextColor3 = newlib.themes.text
    ...
    -- expose simple notification
    newlib.notifications = {}
    function newlib.notifications:create_notification(cfg)
        cfg = cfg or {}
        local s = new("ScreenGui"); parentGui(s)
        local frame = new("Frame", s)
        frame.Size = UDim2.new(0,320,0,64)
        ...
        txt.Position = UDim2.new(0, 12, 0, 6)
        txt.BackgroundTransparency = 1
        txt.Text = (cfg.name or "Notice").."\n"..(cfg.info or "")
        txt.TextColor3 = newlib.themes.text
        txt.Font = Enum.Font.SourceSans
        spawn(function() wait(cfg.lifetime or 4) pcall(function() s:Destroy() end) end)
    end

    -- store in global
    getgenv().cyber_valley_ui = newlib
end
