...
-- \\ Script
-- Orion UI rewrite for Nameless Hub
...
-- Tabs (mapped roughly to the old categories)
...
-- Helper for safe function calls
...
local Config = Config or getgenv().Config or {}
local TheBronxConfig = Config.TheBronx or {}
local WorldVisuals = Config.WorldVisuals or {}
local MiscSettings = Config.MiscSettings or {}
local Aimlock = Config.Aimlock or {}
local Silent = Config.Silent or {}
...
CombatTab:AddToggle({
    ...
    Default = Aimlock.Enabled or false,
    Callback = function(v)
        Aimlock.Enabled = v
...
CombatTab:AddToggle({
    ...
    Default = Silent.Enabled or false,
    Callback = function(v)
        Silent.Enabled = v
...
VisualsTab:AddToggle({
    Name = "Fullbright",
    Default = WorldVisuals.Fullbright or false,
    Callback = function(v)
...
VisualsTab:AddToggle({
    Name = "Custom FOV",
    Default = WorldVisuals.FieldOfViewEnabled or false,
    Callback = function(v)
...
VisualsTab:AddSlider({
    ...
    Min = 60,
    Max = 120,
    Default = WorldVisuals.FieldOfViewValue or 70,
    Callback = function(v)
...
MiscTab:AddToggle({
    ...
    Default = (MiscSettings.ModifySpeed and MiscSettings.ModifySpeed.Enabled) or false,
    Callback = function(v)
        MiscSettings.ModifySpeed = MiscSettings.ModifySpeed or {}
...
MiscTab:AddSlider({
    ...
    Min = 10,
    Max = 250,
    Default = (MiscSettings.ModifySpeed and MiscSettings.ModifySpeed.Value) or 50,
    Callback = function(v)
        MiscSettings.ModifySpeed = MiscSettings.ModifySpeed or {}
...
MiscTab:AddToggle({
    Name = "Modify Jump",
    Default = (MiscSettings.ModifyJump and MiscSettings.ModifyJump.Enabled) or false,
    Callback = function(v)
        MiscSettings.ModifyJump = MiscSettings.ModifyJump or {}
...
MiscTab:AddSlider({
    ...
    Min = 25,
    Max = 200,
    Default = (MiscSettings.ModifyJump and MiscSettings.ModifyJump.Value) or 50,
    Callback = function(v)
        MiscSettings.ModifyJump = MiscSettings.ModifyJump or {}
...
-- // Teleport tab (The Bronx locations, if available)

local teleportOptions = {}
if TheBronxConfig.TeleportationList then
    for name, cf in pairs(TheBronxConfig.TeleportationList) do
        table.insert(teleportOptions, name)
    end
end
table.sort(teleportOptions)

local selectedTeleport = TheBronxConfig.Selected_Location or "..."

TeleportTab:AddDropdown({
    Name = "Teleport Location",
    Default = selectedTeleport ~= "..." and selectedTeleport or nil,
    Options = teleportOptions,
    Callback = function(v)
        selectedTeleport = v
        TheBronxConfig.Selected_Location = v
...
TeleportTab:AddButton({
    Name = "Teleport",
    Callback = function()
        if not (TheBronxConfig.TeleportationList and selectedTeleport and TheBronxConfig.TeleportationList[selectedTeleport]) then
            return
        end
        ...
        if hrp then
            hrp.CFrame = TheBronxConfig.TeleportationList[selectedTeleport]
        end
    end
})
