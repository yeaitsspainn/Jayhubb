...
-- MOBILE SAFE WALKSPEED PATCH (Injected)
task.spawn(function()
    while task.wait(0.05) do
        local lp = game.Players.LocalPlayer
        local char = lp.Character
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        if hum and Config and Config.MiscSettings and Config.MiscSettings.ModifySpeed then
            if Config.MiscSettings.ModifySpeed.Enabled then
                local speed = Config.MiscSettings.ModifySpeed.Value or 16
