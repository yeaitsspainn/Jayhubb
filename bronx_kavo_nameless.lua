...
-- // Nameless Hub Kavo UI for The Bronx (Delta compatible, all config fields auto-wired)
...
if Game_Name == "The Bronx" then
    ...
    if success and Library then
        local Window = Library.CreateLib("Nameless Hub | The Bronx", "Midnight")
        ...
        local function addBoolSection(tab, sectionName, tbl)
            if type(tbl) ~= "table" then return end
            local sec = tab:NewSection(sectionName)
            for key, val in pairs(tbl) do
                if type(val) == "boolean" then
                    sec:NewToggle(prettify(key), "Toggle for "..tostring(key), function(state)
                        tbl[key] = state
                    end)
                end
            end
        ...
        local function addNumberSection(tab, sectionName, tbl, minDefault, maxDefault)
            if type(tbl) ~= "table" then return end
            local sec = tab:NewSection(sectionName)
            for key, val in pairs(tbl) do
                if type(val) == "number" then
                    local minv = 0
                    local maxv = maxDefault or (val * 5)
                    if maxv <= minv then maxv = minv + 100 end
                    sec:NewSlider(prettify(key), "Slider for "..tostring(key), maxv, minv, function(v)
                        tbl[key] = v
                    end)
                end
            end
        ...
        local TheBronx = Config.TheBronx or {}
        ...
        -- // Combat (weapon & player combat mods)
        local CombatSection = CombatTab:NewSection("Weapon Modifications")
        if type(TheBronx._Modifications) == "table" then
            for key, val in pairs(TheBronx._Modifications) do
                if type(val) == "boolean" then
                    CombatSection:NewToggle(prettify(key), "Toggle "..tostring(key), function(state)
                        TheBronx._Modifications[key] = state
                    end)
                elseif type(val) == "number" then
                    local maxv = val * 5
                    if maxv < 10 then maxv = 100 end
                    CombatSection:NewSlider(prettify(key), "Value for "..tostring(key), maxv, 0, function(v)
        ...
        local PlayerCombatSection = CombatTab:NewSection("Player Combat Utilities")
        if type(TheBronx.PlayerUtilities) == "table" then
            for key, val in pairs(TheBronx.PlayerUtilities) do
                if type(val) == "boolean" then
                    PlayerCombatSection:NewToggle(prettify(key), "Toggle "..tostring(key), function(state)
                        TheBronx.PlayerUtilities[key] = state
                    end)
                end
            end
        end

        -- // Visuals (ESP + world visuals)
        local EspSection = VisualsTab:NewSection("ESP")
        EspSection:NewToggle("ESP Enabled", "Toggle ESP", function(state)
            Config.ESP.Enabled = state
            if typeof(RefreshAllElements) == "function" then
                pcall(RefreshAllElements)
            end
        end)

        local WorldSection = VisualsTab:NewSection("World Visuals")
        WorldSection:NewToggle("Fullbright", "Toggle fullbright", function(state)
            Config.WorldVisuals.Fullbright = state
        end)
        WorldSection:NewToggle("Custom FOV", "Toggle FOV override", function(state)
            Config.WorldVisuals.FieldOfViewEnabled = state
        end)
        WorldSection:NewSlider("FOV Value", "Camera field of view", 120, 60, function(v)
            Config.WorldVisuals.FieldOfViewValue = v
        end)

        -- // Movement (walkspeed / jump)
        local MoveSection = MovementTab:NewSection("Movement Mods")
        Config.MiscSettings.ModifySpeed = Config.MiscSettings.ModifySpeed or {Enabled = false, Value = 50}
        Config.MiscSettings.ModifyJump  = Config.MiscSettings.ModifyJump  or {Enabled = false, Value = 50}

        MoveSection:NewToggle("Modify WalkSpeed", "Enable custom walk speed", function(state)
            Config.MiscSettings.ModifySpeed.Enabled = state
        end)
        MoveSection:NewSlider("WalkSpeed %", "Walkspeed percentage", 250, 10, function(v)
            Config.MiscSettings.ModifySpeed.Value = v
        end)

        MoveSection:NewToggle("Modify Jump", "Enable custom jump power", function(state)
            Config.MiscSettings.ModifyJump.Enabled = state
        end)
        MoveSection:NewSlider("Jump Power", "Jump strength", 200, 25, function(v)
            Config.MiscSettings.ModifyJump.Value = v
        end)

        -- // Farms (all farm-related booleans)
        if type(TheBronx.Farms) == "table" then
            local FarmSection = FarmsTab:NewSection("Farms & Money")
            for key, val in pairs(TheBronx.Farms) do
                if type(val) == "boolean" then
                    FarmSection:NewToggle(prettify(key), "Toggle "..tostring(key), function(state)
        ...
        local selectedTeleport = TheBronx.Selected_Location or "..."

        TeleportSection:NewDropdown("Location", "Select teleport destination", teleportNames, function(choice)
            selectedTeleport = choice
            TheBronx.Selected_Location = choice
        end)

        TeleportSection:NewButton("Teleport", "Teleport to selected location", function()
            if TheBronx.TeleportationList and selectedTeleport and TheBronx.TeleportationList[selectedTeleport] then
                local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
                local hrp = char:FindFirstChild("HumanoidRootPart")
                if hrp then
                    hrp.CFrame = TheBronx.TeleportationList[selectedTeleport]
                end
            end
        end)

        -- // Misc tab: expose remaining player modifications as toggles
        if type(TheBronx.PlayerModifications) == "table" then
            local PMSection = MiscTab:NewSection("Player Modifications")
            for key, val in pairs(TheBronx.PlayerModifications) do
                if type(val) == "boolean" then
                    PMSection:NewToggle(prettify(key), "Toggle "..tostring(key), function(state)
        ...
        warn("[Nameless Hub] Failed to load Kavo UI Library.")
    end
end
